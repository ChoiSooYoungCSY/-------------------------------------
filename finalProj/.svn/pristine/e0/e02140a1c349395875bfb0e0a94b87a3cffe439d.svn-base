...
<div class="bdy col-span-12 lg:caol-span-9 2xl:col-span-10">
	<form id="frm" method="get" enctype="multipart/form-data">
		<div class="intro-y news p-5 box mt-0 pb-16">
			<h2 class="intro-y font-medium text-xl sm:text-2xl flex items-center"
				style="margin-left: 15px;">
				${sttPostDetail[0].pstSj}&nbsp; <span
					class="bg-red-700 bg-opacity-80 text-white text-sm m-1 px-2 py-1 rounded z-10">신고
					횟수 : ${sttCnt.cnt}</span> <a id="btnDelete"
					class="btn btn-primary ml-auto text-lg font-bold py-1"
					data-info="${sttPostDetail[0].sttemntPstId}">삭제</a> <a
					onclick="javascript:history.back()"
					class="btn btn-outline-primary ml-2 text-lg font-bold py-1">취소</a>
			</h2>
			<br />
			<hr />
			...
			<div
				class="intro-y flex relative mt-10 py-4 items-center border-t border-slate-300/60">
				<div class="w-full flex text-slate-700 ml-3">
					<div class="text-lg font-bold">
						작성자 : ${sttPostDetail[0].frstWrterId}<span class="font-medium"></span>
					</div>
				</div>
			</div>
			<div
				class="intro-y flex sm:text-sm flex-col sm:flex-row items-center pt-5 border-t border-slate-200/60 ml-3">
				<div class="flex items-center">
					<div>
						<h6 class="font-medium text-lg leading-none">
							판매가 :
							<fmt:formatNumber value='${sttPostDetail[0].scndhandPc}'
								type="number" />
							원
						</h6>
					</div>
				...
				<div class="flex items-center ml-auto">
					<h6 class="font-medium text-lg leading-none">조회수 :
						${sttPostDetail[0].rdcnt}</h6>
				</div>
			</div>
			<div class="intro-y mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4 ml-3">
				<c:forEach var="atchmnflDetailVO"
					items="${sttPostDetail[0].atchmnflDetailList}" varStatus="stat">
					<div class="nEWS__PREVIEW IMAGE-FIT">
						<img alt="Midone - HTML Admin Template" class="rounded-md"
							src="/resources/upload${atchmnflDetailVO.flpth}/${atchmnflDetailVO.streFlNm}">
					</div>
				</c:forEach>
			</div>
